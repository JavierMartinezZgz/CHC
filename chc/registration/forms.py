from django import forms
from django.contrib.auth.forms import UserCreationForm
from django.contrib.auth.models import User
from .models import Profile

class UserCreationFormWithEmail(UserCreationForm):
    email = forms.EmailField(required=True, help_text="Requerido, 254 caracteres como maximo y debe ser válido")

    class Meta:
        model = User
        fields = ("username", "email", "password1", "password2")

    def clean_email(self): #Validar el campo email. Hay que usar esta estructura clean_campo
        email = self.cleaned_data.get("email") #Recogemos el valor del campo email
        if User.objects.filter(email=email).exists(): #Comprueba si el email ya está usado en los usuarios
            raise forms.ValidationError("El email ya está registrado")
        return email 

class ProfileForm(forms.ModelForm):
    class Meta:
        model = Profile
        fields = ['avatar', 'bio', 'link']
        widgets = {
            'avatar': forms.ClearableFileInput(attrs={'class':'form-control-file mt-3'}),
            'bio': forms.Textarea(attrs={'class':'form-control-file mt-3', 'rows':3, 'placeholder':'Biografía'}),
            'link': forms.URLInput(attrs={'class':'form-control-file mt-3', 'placeholder':'Enlace'}),
        }

class EmailForm(forms.ModelForm):
    email = forms.EmailField(required=True, help_text="Requerido, 254 caracteres como maximo y debe ser válido")

    class Meta:
        model = User
        fields = ['email']

    def clean_email(self): #Validar el campo email. Hay que usar esta estructura clean_campo
        email = self.cleaned_data.get("email") #Recogemos el valor del campo email
        if 'email' in self.changed_data:
            if User.objects.filter(email=email).exists(): #Comprueba si el email ya está usado en los usuarios
                raise forms.ValidationError("El email ya está registrado")
        return email 
